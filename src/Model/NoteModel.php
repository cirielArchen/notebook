...
class NoteModel extends AbstractModel implements ModelInterface
{
    ...
    public function get(int $id): array
    {
        ...
        if (!$note) {
            throw new NotFoundException("Notatki o id: $id nie istnieje");
        }
    ...
    {
        try {
            //zrobić validację czy pole nie jest puste, czy nie jest za długie, minimalna długość
    ...
    private function findBy(?string $phrase, int $pageNumber, int $pageSize, string $sortBy, string $sortOrder): array
    {
        try {
            ...
            $query = "SELECT id, title, created FROM notes $wherePart ORDER BY $sortBy $sortOrder LIMIT $offset, $limit";
